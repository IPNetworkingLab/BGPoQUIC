...
static void
bgp_dump_message(struct bgp_conn *conn, byte *pkt, uint len, int is_local)
{
  ...
  mrt_dump_bgp_message(&d, is_local);
}

static void
bgp_dump_message_et(struct bgp_conn *conn, byte *pkt, uint len, int is_local) {
    struct mrt_bgp_data d;
    init_mrt_bgp_data(conn, &d);
    ...
    mrt_dump_bgp_message_et(&d, is_local);
}
...
bgp_send(struct bgp_conn *conn, uint type, uint len)
{
  ...
  if (conn->bgp->p.mrtdump & MD_MESSAGES) {
    if (conn->bgp->p.mrtdump_et)
      bgp_dump_message_et(conn, buf, len, 1);
    else
      bgp_dump_message(conn, buf, len, 1);
  }
...
bgp_rx_packet(struct bgp_conn *conn, byte *pkt, uint len)
{
  ...
  if (conn->bgp->p.mrtdump & MD_MESSAGES)
      if (conn->bgp->p.mrtdump_et)
        bgp_dump_message_et(conn, pkt, len, 0);
      else
        bgp_dump_message(conn, pkt, len, 0);
