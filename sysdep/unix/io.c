...
size_t sk_get_remote_tls_certificate(sock *sk, void *pem_cert, size_t pem_cert_len) {
    if (!(sk->type == SK_QUIC_ACTIVE_CONNECTED) && !(sk->type == SK_QUIC_PASSIVE_CONNECTED)) {
        return -1;
    }

    return picoquic_get_remote_certificate(sk->fd, pem_cert, pem_cert_len);
}
...
int sk_set_fallback_address(sock *s, ip_addr *from, ip_addr *to,
                            u16 from_port, u16 to_port) {
    struct sockaddr_storage _from, _to;

    /* connection migration is only
     * initiated from the client */
    if (s->type != SK_QUIC_ACTIVE_CONNECTED) {
        return -1;
    }
    ...
    if (ipa_is_ip4(*from) && ipa_is_ip4(*to)) {
        ((struct sockaddr_in *) &_from)->sin_family = AF_INET;
        ((struct sockaddr_in *) &_from)->sin_addr = ipa_to_in4(*from);
        ((struct sockaddr_in *) &_from)->sin_port = htons(from_port);

        ((struct sockaddr_in *) &_to)->sin_family = AF_INET;
        ((struct sockaddr_in *) &_to)->sin_addr = ipa_to_in4(*to);
        ((struct sockaddr_in *) &_to)->sin_port = htons(to_port);
    } else if (ipa_is_ip6(*from) && ipa_is_ip6(*to)) {
        ((struct sockaddr_in6 *) &_from)->sin6_family = AF_INET6;
        ((struct sockaddr_in6 *) &_from)->sin6_addr = ipa_to_in6(*from);
        ((struct sockaddr_in6 *) &_from)->sin6_port = htons(from_port);

        ((struct sockaddr_in6 *) &_to)->sin6_family = AF_INET6;
        ((struct sockaddr_in6 *) &_to)->sin6_addr = ipa_to_in6(*to);
        ((struct sockaddr_in6 *) &_to)->sin6_port = htons(to_port);
    } else {
        /* from & to are not from the same AF */
        return -1;
    }

    return picoquic_set_fallback_address(s->fd, (struct sockaddr*) &_from,
                                         (struct sockaddr *) &_to);
}
...
int sk_open_stream(sock *sk, void (*tx_hook)(struct birdsock *)) {
    ...
    t->tbsize = sk->tbsize;
    t->iface = sk->iface;
    t->lifindex = t->iface ? t->iface->index : sk->lifindex;
    t->laddr = sk->laddr;
    t->faddr = sk->daddr;
    log("here1 %I", sk->laddr);
...
sk_passive_connected(sock *s, int type)
{
  ...
  if (type == SK_TCP)
  {
    ...
    if (sockaddr_read(&rem_sa, s->af, &t->daddr, &t->iface, &t->dport) < 0)
      ...
      t->dport = s->dport;
      t->iface = s->iface;
      /* @Nico: the line t->lifindex = t->iface->index
       * causes segfault on BGP side..
       * FIXME: check if this modification is ok on OSPF... */
